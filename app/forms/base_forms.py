...
from wtforms import StringField, SelectField, TextField, validators, \
 PasswordField, BooleanField, ValidationError, DecimalField
...
class AddSessionForm(FlaskForm):
    ...
    tutorName = StringField('Tutor Name',[validators.Length(min=4, max=40),
                                                  validators.Regexp('[A-Za-z0-9]+',
                                                                    message="Please enter your first name")], render_kw={'placeholder': 'First Name'})

    sessionDate = StringField('Last Name',[validators.Length(min=3, max=40),
                                                        validators.Regexp('[A-Za-z0-9]+',
                                                                          message="Please enter your last name")], render_kw={'placeholder':'Last Name'})
    location = TextField('Location',[validators.Length(min=3, max=10),
                                                            validators.Regexp('[0-9]+',
                                                                              message="Please enter your phone number")], render_kw={'placeholder':'Phone Number'})
    type = SelectField(u'Type', choices=[('Walk-In', 'Walk-In'), ('Appointment', 'APPT')])


    subject = SelectField(u'Main Subject', choices=[('MATH', 'Math'), ('ACCT', 'Accounting'), ('AC', 'Applied Communication'), ('SCIENCE', 'Science'), ('GENERAL', 'General')])
...
class RegistrationForm(FlaskForm):
    ...
    username = StringField('Username', [validators.Length(min=3, max=40),
                                                        validators.Regexp('[A-Za-z0-9]+',
                                                                          message="Please enter a username")], render_kw={'placeholder':'UserName'})
...
class AssessmentForm(FlaskForm):
    pass
class RequestTutorForm(FlaskForm):
    pass
